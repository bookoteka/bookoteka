...
import { DaneRaportuMiesiac } from "../types/raporty";

interface SzablonRaportMiesiacProps {
  dane: DaneRaportuMiesiac;
}

export default function SzablonRaportMiesiac({ dane }: SzablonRaportMiesiacProps) {
  return (
    <div className="bg-white text-slate-800 font-sans p-6" style={{ width: "794px", minHeight: "1123px" }}>
      ...
      <header className="border-b-2 border-slate-900 pb-3 mb-5 flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">{dane.naglowek} – podsumowanie</h1>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {dane.ksiazki.length > 0 ? (
                ...
                <tr>
                  <td colSpan={8} className="py-3 px-2.5 text-center text-slate-400">Brak przeczytanych książek w tym miesiącu.</td>
                </tr>
              )}
            </tbody>
          </table>
